                                         ...
                                         const bool asyncExecution)
{
    ...
    this->decoderPipeline->AddOperator(dali::OpSpec("Copy")
                                           .AddInput("decoded_images", "cpu")
                                           .AddOutput("decoded_images_copy0", "cpu"),
                                       "JPEGHostDecoder_copy");

    this->decoderPipeline->AddOperator(dali::OpSpec("Copy")
                                           .AddInput("decoded_images", "cpu")
                                           .AddOutput("decoded_images_copy1", "cpu"),
                                       "JPEGHostDecoder_copy");

    std::cout << "Setting up JPEG decoder" << std::endl;
    this->outputs = {{"decoded_images_copy0", "cpu"},
                     {"decoded_images_copy1", "cpu"}};
}
...
void JPEGDecoderPipeline::GetPipelineOutput(std::vector<dali::TensorList<dali::CPUBackend>*>& tls1,
                                            std::vector<dali::TensorList<dali::CPUBackend>*>& tls2)
{
    this->decoderPipeline->Outputs(&this->ws);
    tls1.push_back(&ws.Output<dali::CPUBackend>(0));
    tls2.push_back(&ws.Output<dali::CPUBackend>(1));
}
