...
template <typename T>
void rotate(T *output, T *input, T *angle, T *center, dim_t *input_dims,
            RotateInterpolation interp, cudaStream_t stream) {
  int channel = (int)input_dims[0];
  int height = (int)input_dims[1];
  int width = (int)input_dims[2];
...
template void rotate(float *output, float *input, float *angle, float *center,
                     dim_t *input_dims, RotateInterpolation interp,
                     cudaStream_t stream);

template void rotate(__half *output, __half *input, __half *angle,
                     __half *center, dim_t *input_dims,
                     RotateInterpolation interp, cudaStream_t stream);
