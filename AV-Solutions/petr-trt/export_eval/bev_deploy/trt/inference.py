...
class InferTrt(object):
    ...
    def forward(self, args, stream=None):
        stream_in_use = self.stream.handle if stream is None else stream.cuda_stream
        # args are torch.Tensor.data_ptr(), please always beware of args' dtype
        self.cuda_ctx.push()
        for i in range(len(self.names)):
            self.context.set_tensor_address(self.names[i], args[i])
        self.context.execute_async_v3(stream_handle=stream_in_use)
        # self.stream.synchronize()
        self.cuda_ctx.pop()
        return
    
    def sync(self):
        # only call this when not using torch's stream
        self.stream.synchronize()
