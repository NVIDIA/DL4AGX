...
class Hook(object):
    ...
    def init(self, fn):
        ...
        if isinstance(fn, functools.partial):
            self.fname = f"partial<{fn.func.__code__.co_name}>"
            fn = fn.func
        else:
    ...
    def _wrap(self, func):
        ...
        def _inner_func(*args, **kwargs):
            # NOTE: args might have no element
            if len(args) != 0:
                me.before(args[0])
            ...
            # typically id(args[0]) == id(me.func.__self__)
            if id(args[0]) != id(me.func.__self__) and type(args[0]) == type(me.func.__self__):
                # in this case, somehow the caller changed unexpectly
                if isinstance(me.func.__self__, torch.nn.Module):
                    # this is not expected, but somehow working by manually set requries_grad to False
                    # make sure you know what you do here
                    mod = me.func.__self__
                ...
                if (_fn_name not in _named_mods) and (_fn_name not in _built_ins):
                    ...
                    if _fn_name not in self._fn_attached[mid]:
                        ...
                        if isinstance(_fn, types.MethodType):
                            ...
                            # print(f"hooking {mod.__class__}.{_fn_name}, {prefix}, {_fn_name_lut[_fn_name]}")
                        elif isinstance(_fn, types.FunctionType):
                            hook = Hook(mod, _fn_name, prefix)
                            self.hooks[hook.key] = hook
                            # print(f"hooking function {mod.__class__}.{_fn_name}, {prefix}, {_fn_name_lut[_fn_name]}")
            return
