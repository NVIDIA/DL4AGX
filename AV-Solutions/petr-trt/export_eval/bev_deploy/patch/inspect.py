...
class StatefulModuleTrt(torch.nn.Module):
...
class AutoInspectHelper(object):
    ...
    def check_requires_grad(model):       
        ...
        for name, buffer in model.named_buffers():
            print(f"Buffer {name}: requires_grad = {buffer.requires_grad}")

    def export_onnx(self, model_trt, args, input_names, output_names, session=None):
        os.umask(0)        
        os.makedirs(f"{session}", exist_ok=True)

        with io.BytesIO() as f, torch.no_grad() as _:
            model_trt.eval()
            AutoInspectHelper.check_requires_grad(model_trt)

            torch.onnx.export(
                model_trt, tuple([a.detach() for a in args]), f,
            ...
            # onnx.save(onnx_mod, f"{session}/{self.hook.key}.onnx")
    ...
    def export(self, session=None):
        session = f"scratch/{self.hook.key}" if session is None else session
        os.makedirs(f"{session}/{self.hook.key}", exist_ok=True)
        ...
        for n, t in zip(input_names, args):
            if isinstance(t, torch.Tensor):
                ...
                if t.dtype == torch.float64:
                    t = t.to(torch.float32)
                t.cpu().detach().numpy().tofile(f"{session}/{self.hook.key}/{n}.bin")
            elif isinstance(t, float):
                print(n, "float")
                np.array([t], dtype=np.float32).tofile(f"{session}/{self.hook.key}/{n}.bin")
            else:
                pass
    ...
    def export(self, session=None):
        ...
        for n, t in zip(input_names, args):
            if isinstance(t, torch.Tensor):
                ...
                if t.dtype == torch.float64:
                    t = t.to(torch.float32)
                t.cpu().detach().numpy().tofile(f"{session}/{n}.bin")
            elif isinstance(t, float):
                print(n, "float")
                np.array([t], dtype=np.float32).tofile(f"{session}/{n}.bin")
            else:
                pass
