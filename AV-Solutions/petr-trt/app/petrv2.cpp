...
int main(int argc, char** argv) {
  ...
  // since petrv2 is a temporal model, we pre-calculate the feature for the first frame
  nets["backbone"]->bindings["img"]->load(data_dir + "imgs/img_0.bin");
  nets["backbone"]->Enqueue(stream);
  cudaStreamSynchronize(stream);
  void* p0 = nets["backbone"]->bindings["prev.0"]->ptr;
  void* o0 = nets["backbone"]->bindings["out.0"]->ptr;
  void* p1 = nets["backbone"]->bindings["prev.1"]->ptr;
  void* o1 = nets["backbone"]->bindings["out.1"]->ptr;

  nets["head"]->bindings["img_metas.0[mean_time_stamp]"]->load(data_dir + "v2_mean_time_stamp.bin");
  nets["head"]->bindings["img_metas.0[coords_position_embeding]"]->load(data_dir + "v2_coords_pe.bin");
  ...
  for( int frame_id=1; frame_id < n_frames; frame_id++ ) {
    ...
    // here we directly copy feature from previous round
    // prev.i[...] = out.i[0, 0:6, 256, H, W]
    cudaMemcpyAsync(p0, o0, sizeof(float) * 6 * 256 * 20 * 50, cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(p1, o1, sizeof(float) * 6 * 256 * 10 * 25, cudaMemcpyDeviceToDevice, stream);
    nets["backbone"]->Enqueue(stream);
    nets["head"]->Enqueue(stream);
    cudaStreamSynchronize(stream);
    ...
    printf("[INFO] %d finished, n_elem = %d\n", frame_id, n_elem);
  }
